...
//   const [availableVariants, setAvailableVariants] = useState<Array<{value: string, label: string}>>([]);
...
//   // Fetch PDF details and product variants
...
//           const variantIdsWithPdfs = productWithVariants.pdfs.map(pdf => pdf.variantId);

//           const withPdfs: Variant[] = [];
//           const withoutPdfs: Variant[] = [];

//           productWithVariants.variants.forEach(variant => {
//             const hasPdf = variantIdsWithPdfs.includes(variant.variantId);
...
//           const variantOptions = withoutPdfs.map(v => ({
//             value: v.variantId,
//             label: `${v.variantTitle}${v.variantPrice ? ` - ₹${v.variantPrice}` : ''}`,
//             inventory: v.inventory
//           }));

//           if (foundPdf.variantId) {
//             const currentVariant = productWithVariants.variants.find(v => v.variantId === foundPdf.variantId);
//             if (currentVariant) {
//               const alreadyInOptions = variantOptions.some(opt => opt.value === foundPdf.variantId);
//               if (!alreadyInOptions) {
//                 variantOptions.unshift({
//                   value: foundPdf.variantId,
...
//     options.push({ value: "", label: "Select Product variant" });
...
//     if (pdf.variantId) {
//       const currentVariant = productVariants.find(v => v.variantId === pdf.variantId);
//       if (currentVariant) {
//         const alreadyInOptions = options.some(opt => opt.value === pdf.variantId);
//         if (!alreadyInOptions) {
//           options.push({
//             value: pdf.variantId,
//             label: `${currentVariant.variantTitle}${currentVariant.variantPrice ? ` - ₹${currentVariant.variantPrice}` : ''} (Current)`,
...
//   // Step 3: Handle PDF file upload with validation
...
//     const existingUpload = uploadedPDFs.find(pdf => pdf.variantId === selectedVariant);
//     if (existingUpload) {
//       setError(`Variant already has a PDF uploaded. Please remove "${existingUpload.name}" first or select a different variant.`);
//       if (fileInputRef.current) {
//         fileInputRef.current.value = "";
//       }
//       return;
...
//     const existingAttachment = existingAttachments.find(att => att.variantId === selectedVariant);

//     // Get variant details for the uploaded PDF
//     const variantDetails = productVariants.find(v => v.variantId === selectedVariant);
...
//       setError("");
//       setSuccess(`PDF "${file.name}" added for ${variantDetails?.variantTitle}`);
...
//       // Remove any existing uploaded PDF for this variant (shouldn't exist, but just in case)
//       setUploadedPDFs(prev => prev.filter(pdf => pdf.variantId !== pendingUpload.variantId));

//       // Add the new PDF
//       setUploadedPDFs(prev => [...prev, pendingUpload]);

//       // Track PDFs to be replaced
//       const pdfToReplace = existingAttachments.find(att => att.variantId === pendingUpload.variantId);
//       if (pdfToReplace) {
//         setPdfsToReplace(prev => [...prev.filter(p => p.id !== pdfToReplace.id), pdfToReplace]);
//       }

//       setSuccess(`PDF "${pendingUpload.name}" will replace existing PDF for this variant`);
//       setTimeout(() => setSuccess(""), 3000);
//     }
...
//       const wasReplacing = existingAttachments.find(att => att.variantId === pdfToRemove.variantId);
//       if (wasReplacing) {
//         setPdfsToReplace(prev => prev.filter(p => p.variantId !== pdfToRemove.variantId));
//       }
//     }
...
//   // Step 3: Handle upload and save with validation
...
//   // Step 4: Final save after review
//   // const handleFinalSave = async () => {
//   //   if (!pdf || !selectedProduct) {
//   //     setError("Missing PDF or product information");
//   //     return;
//   //   }
...
//   //     // Prepare variant data array matching API expectations
//   //     const variantMappings: any[] = uploadedPDFs.map((uploadedPdf) => {
//   //       const variantInfo = productVariants.find(
//   //         (v) => v.variantId === uploadedPdf.variantId
//   //       );
//   //       return {
//   //         variantId: uploadedPdf.variantId,
//   //         variantTitle: variantInfo?.variantTitle || uploadedPdf.variantTitle || "",
//   //         variantPrice: variantInfo?.variantPrice || "",
//   //       };
//   //     });

//   //     // Prepare all variants data
//   //     const allVariantsData = productVariants.map((variant) => ({
//   //       value: variant.variantId,
...
//   //     formData.append("productId", pdf.productId);
...
//   //       formData.append("pdfs", uploadedPdf.file);
//   //     });

//   //     // Add variant data as JSON string
//   //     formData.append("variantData", JSON.stringify(variantMappings));
//   //     formData.append("allVariants", JSON.stringify(allVariantsData));

//   //     // Add PDFs to replace (if any) - this tells API to remove old PDFs before uploading
//   //     if (pdfsToReplace.length > 0) {
//   //       formData.append("pdfsToReplace", JSON.stringify(pdfsToReplace));
//   //     }

//   //     const response = await fetch(`/api/product-pdfs/${pdf.id}/update`, {
//   //       method: "POST",
//   //       body: formData,
//   //     });
...
//   //     if (result.success) {
//   //       setSuccess(`${uploadedPDFs.length} PDF(s) uploaded successfully!`);
//   //       setUploadedPDFs([]);
//   //       setPdfsToReplace([]);
...
//   //       setTimeout(() => setSuccess(""), 3000);
//   //     } else {
//   //       setError(result.message || result.error || "Failed to upload PDFs");
//   //       setShowReviewModal(false);
//   //     }
//   //   } catch (err) {
//   //     setError("Error uploading PDFs");
//   //     setShowReviewModal(false);
//   //     console.error(err);
//   //   } finally {
//   //     setUploading(false);
...
//     if (!selectedVariant) {
//       setError("Please select a variant to assign the PDF");
//       return;
//     }
...
//       formData.append("variantTitle", selectedVariantDetails?.variantTitle || "");
//       formData.append("variantPrice", selectedVariantDetails?.variantPrice || "");

//       // Include all variants for backend sync
//       if (productVariants.length > 0) {
//         formData.append("allVariants", JSON.stringify(productVariants.map(v => ({
//           value: v.variantId,
...
//       // If editing an existing PDF, include its ID
//       if (pdf?.id) {
//         formData.append("pdfId", pdf.id);
//       }

//       // Look for a newly uploaded file
...
//         const uploadedFile = uploadedPDFs[0].file;
//         formData.append("pdf", uploadedFile);
//       }

//       // DEBUG
//       console.log("Using ProductPDF ID:", selectedProduct.id);
//       console.log("Individual PDF ID (if editing):", pdf?.id);

//       // Use the ProductPDF ID
//       const response = await fetch(`/api/product-pdfs/${selectedProduct.id}/update`, {
//         method: "PUT",
//         body: formData,
//       });
...
//       if (result.success) {
//         setSuccess(result.message || "PDF saved successfully!");
...
//         if (result.existingPdfId) {
//           setError(prev => prev + " Would you like to edit it instead?");
//         }
//       }
//     } catch (err) {
...
//   //       (v) => v.variantId === selectedVariant
//   //     );
//   //     const response = await fetch("/api/product-pdfs/variants/edit", {
//   //       method: "PUT",
//   //       headers: {
...
//   //         variantTitle: selectedVariantDetails?.variantTitle || undefined,
//   //         variantPrice: selectedVariantDetails?.variantPrice || undefined,
//   //       }),
//   //     });
//   //     const result = await response.json();
...
//   //         name: pdfName.trim(),
//   //         variantId: selectedVariant,
//   //         variantTitle: selectedVariantDetails?.variantTitle || "",
//   //         variantPrice: selectedVariantDetails?.variantPrice || "",
//   //       });
...
//       console.log("ProductPDF ID (selectedProduct.id):", selectedProduct.id);
//       console.log("Individual PDF ID (pdf.id):", pdf.id);
//       console.log("Selected variant:", selectedVariant);

//       // Use the ProductPDF ID, not the individual PDF ID
//       const response = await fetch(`/api/product-pdfs/${selectedProduct.id}/update`, {
//         method: "PUT",
//         body: formData,
//       });
...
//         }, 1500);
//       } else {
//         setError(result.message || result.error || "Failed to update PDF");
//       }
//     } catch (err) {
//       setError("Error updating PDF");
//       console.error(err);
...
//       <Page
//         title="Edit PDF"
//         backAction={{ content: "Back to Library", onAction: () => router.push("/pdf") }}
//       >
//         <Box padding="800">
...
//                   onChange={setPdfName}
//                   autoComplete="off"
//                   error={error && !pdfName.trim() ? "PDF name is required" : undefined}
...
//             <Banner tone="warning">
//               <Text as="p" variant="bodyMd">
//                 This variant already has a PDF attached. The new PDF will replace the existing one.
//               </Text>
//             </Banner>
...
//                                   <Text as="p" variant="bodySm">{att.name}</Text>
...
//                           </Text>
//                           <InlineStack gap="100">
//                             <Text as="p" variant="bodySm">{pendingUpload.name}</Text>
...
export default function EditPDFPage() {
  ...
  if (!shop) {
    setError("Shop information missing. Please reload the app.");
    return;
  }
